...
/**
 * This object will keep value that will care value remain(can't negative value although it was sent before)
 */
public class ObjectWrapper {
    ...
    public ObjectWrapper(ContactModel config, Cursor cNumber, Cursor cKeep) {
    ...
    public ObjectWrapper(ContactModel config) {
        this.toInsert = new HashMap<>();
    ...
    public int putKeep(int price, String number, String type) {
        ...
        if (current.containsKey(key))
        {
            ...
            {
                //If keep > received - sent => set keep = received - sent.
                cur[1] = cur[0] - cur[2];
                keep.put(key, new int[]{price + cur[1] - oldKeep});
            }
            current.put(key, cur);
        }
        else if (toInsert.containsKey(key))
    ...
    /**
     * Use for keep xien in keep by value provided.
     * It's not reference to percent keep with each user.
     */
